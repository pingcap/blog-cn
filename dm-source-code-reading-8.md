...
本文为 DM 源码阅读系列文章的第八篇，[上篇文章](https://mp.weixin.qq.com/s/CLwEzKt-tOaXmQk1AiuKww) 对 DM 中的定制化数据同步功能进行详细的讲解，包括库表路由（Table routing）、黑白名单（Black & white table lists）、列值转化（Column mapping）、binlog 过滤（Binlog event filter）四个主要功能的实现。
...
![](media/dm-source-code-reading-8/1.jpg)
...
![](media/dm-source-code-reading-8/2.jpg)
