...
[`Titan`](https://github.com/pingcap/rocksdb/tree/titan-5.15) 是由 [`PinCAP`](https://www.pingcap.com/) 研发的一个基于  [`RocksDB`](https://github.com/facebook/rocksdb) 的高性能单机 key-value 存储引擎，其主要设计灵感来源于`USENIX FAST 2016` 上发表的一篇论文 [`WiscKey`](https://www.usenix.org/system/files/conference/fast16/fast16-papers-lu.pdf)。`WiscKey` 提出了一种高度基于 SSD 优化的设计，利用 SSD 高效的随机读写性能，通过将 value 分离出 `LSM-tree` 的方法来达到降低写放大的目的。
