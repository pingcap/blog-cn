...
另外 `lexer` 使用了 `字典树` 技术进行 `token` 识别，具体的实现代码在 [parser/misc.go](https://github.com/pingcap/tidb/blob/rc2.3/parser/misc.go)
...
例如 [plan.preprocess](https://github.com/pingcap/tidb/blob/rc2.3/plan/preprocess.go) 是对 `AST` 做预处理，包括合法性检查以及名字绑定。
...
TiDB 对 `lexer` 和 `parser.go` 进行了封装，对外提供 [parser.yy_parser](https://github.com/pingcap/tidb/blob/rc2.3/parser/yy_parser.go)  进行 SQL 语句的解析：
