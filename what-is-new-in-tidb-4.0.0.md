...
4.0 版本中我们通过 BR（Backup&Restore）工具提供快速备份与恢复数据的功能，备份与恢复的性能高达 1GB/秒，再也不用担心因为数据量过大，无法完成备份与恢复，从根本上解决删库跑路的问题。当研发、DBA 不小心通过 Truncate Table 将数据删除时，我们也贴心的提供 flashback 命令快速恢复被删除的数据。
